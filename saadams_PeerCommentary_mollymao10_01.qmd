...
### NOTE: Sylvie (sadams32) added Molly's comments to this document manually because Molly's edits to the homework were not appearing. Molly sent her comments via Slack.
...
Molly// You are right. Unlist is not needed for this prompt since str_split( alreadt returned the vector.
...
Molly//  I think you did it right. Vectors is better for data that at the same level (for example, different species). Lists are used for data that have subcategories or data within different categories. So for Challenge 4, it is better to use lists since there are subgroups.

3)  I'm not sure when to write install.packages() versus library() when I want a package to be installed as a part of my code.

Molly// So if you write install.packages(), it means this package is installed in your whole R Studio, which can be used always. library() is a script-based code, so you only apply the file/data in your current script. If you want to use the data in your future script, you need to put library(xx) again in your new script. Hope that will be helpful.

4)  I didn't know the best way to change particular features of my theme - I hope it ended up looking okay.

Molly//  I didn't try it also, but I believe you can use theme()!
...
Molly// I also struggled with it on Friday. I believe the first time we tried to push the repo, the github app will send a warming bar and we need to click on it, then the repo will be upload to the webpage successfully.
